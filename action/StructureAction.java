...
public class StructureAction extends ActionSupport implements SessionAware, UserAware {
	...
	public String updateAccount() {
		...
		// Validate password
		if (((this.getPassword().length() <= 5) && (this.getReTyped().length() <= 5)) || this.getPassword().length() > LocandaConstants.STRING_MAX_LENGTH) {
		...
		// Validate maxlength
		if(this.getFirstName().length() > LocandaConstants.STRING_MAX_LENGTH){
			this.getMessage().setResult(Message.ERROR);
			this.getMessage().setDescription(getText("firstNameLengthError"));
			return ERROR;
		}
		if(this.getLastName().length() > LocandaConstants.STRING_MAX_LENGTH){
			this.getMessage().setResult(Message.ERROR);
			this.getMessage().setDescription(getText("lastNameLengthError"));
			return ERROR;
		}
		if(this.getPhone().length() > LocandaConstants.STRING_MAX_LENGTH){
			this.getMessage().setResult(Message.ERROR);
			this.getMessage().setDescription(getText("phoneLengthError"));
			return ERROR;
		}
		
		user.setPhone(this.getPhone());
		user.setName(this.getFirstName());
		user.setSurname(this.getLastName());
