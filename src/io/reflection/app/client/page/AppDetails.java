...
public class AppDetails extends Page implements NavigationEventHandler {
	...
	@UiField
	HeadingElement		trackName;
	@UiField
	Image							image;
	@UiField
	SpanElement				artistName;
	@UiField
	SpanElement				storeName;
	@UiField
	AnchorElement			viewInStore;
	@UiField
	SpanElement				formattedPrice;
	@UiField
	Element						formattedPriceDuplicate;
	@UiField
	Element						categories;
	@UiField
	Element						fileSize;
	@UiField
	Element						languageCodes;
	@UiField
	Element						rated;
	@UiField
	Element						version;
	...
	@UiField
	AnchorElement			developersSite;
	@UiField
	ParagraphElement	developersSiteContainer;
	@UiField
	AnchorElement			appStoreLink;
	@UiField
	ParagraphElement	appStoreLinkContainer;
	@UiField
	AnchorElement			artistStoreLink;
	@UiField
	ParagraphElement	artistStoreLinkContainer;
	@UiField
	Element						appVersionContainer;
	@UiField
	SpanElement				appVersion;
	@UiField
	DivElement				whatsNewContainer;
	@UiField
	SpanElement				releaseNotesText;
	@UiField
	UListElement			screenshotsList;
	@UiField
	UListElement			moreAppsList;
	@UiField
	Element						moreAppsListContainer;
	@UiField
	DivElement				reviewsContainer;
	@UiField
	UListElement			reviewsList;
	@UiField
	DivElement				ratingCurrent;
	@UiField
	DivElement				ratingAll;
	@UiField
	ParagraphElement	moreAppsLink;

	@UiField
	InlineHyperlink	revenueLink;
	@UiField
	Element					premiumIconRevenue;
	@UiField
	InlineHyperlink	downloadsLink;
	@UiField
	Element					premiumIconDownload;
	@UiField
	InlineHyperlink	rankingLink;
	@UiField
	SpanElement			rankingText;
	@UiField
	InlineHyperlink	appDetailsLink;
	@UiField
	SpanElement			appDetailsText;
	@UiField
	LIElement				revenueItem;
	@UiField
	LIElement				downloadsItem;
	@UiField
	LIElement				rankingItem;
	@UiField
	LIElement				appDetailsItem;
	...
	public AppDetails() {
		initWidget(AppDetails.uiBinder.createAndBindUi(this));
		AppDetails.INSTANCE = this;
	...
	public void navigationChanged(Stack previous, Stack current) {
		if (isValidStack(current)) {
			...
			revenueLink.setTargetHistoryToken(PageType.ItemPageType.asTargetHistoryToken(NavigationController.VIEW_ACTION_PARAMETER_VALUE, displayingAppId,
					FilterController.REVENUE_CHART_TYPE, comingPage, previousFilter));
			downloadsLink.setTargetHistoryToken(PageType.ItemPageType.asTargetHistoryToken(NavigationController.VIEW_ACTION_PARAMETER_VALUE, displayingAppId,
					FilterController.DOWNLOADS_CHART_TYPE, comingPage, previousFilter));
			rankingLink.setTargetHistoryToken(PageType.ItemPageType.asTargetHistoryToken(NavigationController.VIEW_ACTION_PARAMETER_VALUE, displayingAppId,
					FilterController.RANKING_CHART_TYPE, comingPage, previousFilter));
			...
			PageType.RanksPageType.show(NavigationController.VIEW_ACTION_PARAMETER_VALUE, FilterController.OVERALL_LIST_TYPE, FilterController.get().asRankFilterString());
		}
	}

	@UiHandler({ "countrySelector" })
	void onFiltersChanged(ChangeEvent event) {
		final String countryCode = countrySelector.getSelectedValue().toString();
		AppDetails.INSTANCE.searchReviews(displayingAppId, countryCode);
		AppDetails.INSTANCE.searchAppForRatingByCountry(displayingAppId, countryCode);
	}
	...
	private String htmlForTextWithEmbeddedNewlines(String text) {
		...
		for (final String line : lines) {
			...
			el.setInnerText(line);
			htmls.add(el.getInnerText());
		}
		final String[] htmlAsArray = new String[htmls.size()];
	...
	private DivElement createReviewArticleDomElement(JsonObject review) {
		final DivElement reviewArticle = Document.get().createDivElement();
		reviewArticle.addClassName(style.customerReview());
		final DivElement ratingsContainer = Document.get().createDivElement();
		ratingsContainer.addClassName(style.ratingsContainer());
		final DivElement ratingsFull = Document.get().createDivElement();
		ratingsFull.addClassName(style.ratingsFull());

		final JsonObject reviewRating = review.getAsJsonObject("im:rating");
		if (reviewRating != null) {
			final Float ratingAsPercentage = reviewRating.get("label").getAsFloat() / 5 * 100; // could
			// use
			// a
			// JSon
			// Utility
		...
		final JsonObject reviewTitle = review.getAsJsonObject("title");
		final HeadingElement reviewTitleDomElement = Document.get().createHElement(3);
		...
		final JsonObject reviewContent = review.getAsJsonObject("content");
		final ParagraphElement reviewParagraphDomElement = Document.get().createPElement();
	...
	private void populateReviews(JsonArray jarray) {
		reviewsList.removeAllChildren();
		for (int r = 1; r < jarray.size() && r < 4; r++) { // first item of array is
			// not a review so start
			// at index 1
			final JsonObject review = jarray.get(r).getAsJsonObject();
			final LIElement listItem = Document.get().createLIElement();

			final DivElement reviewArticle = createReviewArticleDomElement(review);
		...
		if (jarray.size() > 3) {
			...
			viewMoreReviewsButton.setInnerText("View More"); // Update to be
			// multilingual
	...
	private void populateMoreApps(JsonArray jarray) {
		...
		for (int a = 0; a < jarray.size() && a < 16; a++) {
			final JsonObject jobject = jarray.get(a).getAsJsonObject();
			final LIElement listItem = Document.get().createLIElement();
			moreAppsList.appendChild(listItem);

			final AnchorElement aElement = Document.get().createAnchorElement();
			final AnchorElement aElementTitle = Document.get().createAnchorElement();
			aElement.setHref("#!appdetails/" + String.valueOf(jobject.get("trackId")) + "/" + comingPage + "/" + previousFilter); // requires
			// update to add non-static appdetails path
			final ImageElement imgElement = Document.get().createImageElement();
			...
			aElementTitle.setHref("#!appdetails/" + String.valueOf(jobject.get("trackId")) + "/" + comingPage + "/" + previousFilter); // requires
			// update to
			// add
			// non-static
			// appdetails
		...
		if (jarray.size() > 16) {
			final JsonObject firstAppFromMoreAppsList = jarray.get(0).getAsJsonObject();
	...
	private void setRatingsToZero() {
		...
		final DivElement ratingsContainer = Document.get().createDivElement();
		ratingsContainer.addClassName(style.ratingsContainer());
		final DivElement ratingsContainerAll = Document.get().createDivElement();
		ratingsContainerAll.addClassName(style.ratingsContainer());

		final SpanElement numberOfRatings = Document.get().createSpanElement();
		final SpanElement numberOfRatingsAll = Document.get().createSpanElement();
	...
	private void handleAverageRatings(JsonObject data) {
		...
		if (data.get("averageUserRatingForCurrentVersion") != null) {
			final Float ratingAsPercentage = data.get("averageUserRatingForCurrentVersion").getAsFloat() / 5 * 100;
			final DivElement ratingsFull = Document.get().createDivElement();
			ratingsFull.addClassName(style.ratingsFull());
			ratingsFull.getStyle().setProperty("width", ratingAsPercentage + "%");
			ratingsContainer.appendChild(ratingsFull);
			...
			numberOfRatings.setInnerText("No Ratings"); // update so can be
			// non-static multilingual
			// term
			ratingCurrent.appendChild(numberOfRatings);
			ratingCurrent.appendChild(ratingsContainer);
		}
		...
		if (data.get("averageUserRating") != null) {
			final Float ratingAsPercentage = data.get("averageUserRating").getAsFloat() / 5 * 100;
			final DivElement ratingsFull = Document.get().createDivElement();
			ratingsFull.addClassName(style.ratingsFull());
			ratingsFull.getStyle().setProperty("width", ratingAsPercentage + "%");
			...
			numberOfRatingsAll.setInnerText("No Ratings"); // update so can be
			// non-static
			// multilingual term
			ratingAll.appendChild(numberOfRatingsAll);
			ratingAll.appendChild(ratingsContainerAll);
	...
	private void setAppDetails(JsonObject data) {
		...
		final JsonArray screenshotsArray = data.getAsJsonArray("screenshotUrls");
		if (Window.getClientWidth() < 720) {
			imageWidth = 180;
		}

		final String stringToMatch = "320x320";
		final String firstScreenshotUrl = screenshotsArray.get(0).getAsString();
		...
		screenshotsList.removeAllChildren();
		screenshotsList.getStyle().setProperty("width", screenshotsArray.size() * imageWidth - 20 + "px");
		...
		if (data.get("genres") != null) {
			String genres = "";
			final JsonArray categoriesArray = data.getAsJsonArray("genres");
		...
		if (data.get("fileSizeBytes") != null) {
			final int fileSizeBytes = (int) (data.get("fileSizeBytes").getAsFloat() / 1000000);
		...
		if (data.get("description") != null) {
			final String appDescription = htmlForTextWithEmbeddedNewlines(data.get("description").toString().replace("\"", ""));
		...
		if (data.get("features") != null) {
			final JsonArray featuresArray = data.getAsJsonArray("features");
			for (int f = 0; f < featuresArray.size(); f++) {
				final String featureName = featuresArray.get(f).toString().replace("\"", "");
		...
		if (data.get("supportedDevices") != null) {
			String devices = "";
			final JsonArray devicesArray = data.getAsJsonArray("supportedDevices");
		...
		if (data.get("releaseNotes") != null) {
			...
			final String notes = htmlForTextWithEmbeddedNewlines(data.get("releaseNotes").toString().replace("\"", ""));
	...
	private native void search(String searchId) /*-{
		...
		$wnd.$('body').append(
				$wnd.$("<script>").attr("id",
						"ref-searchApiLookupGetAppDetails").attr(
						"src",
	...
	private native void searchMoreApps(String artistName) /*-{
		...
		$wnd
				...
				.append(
						$wnd
								.$("<script>")
								.attr("id", "ref-searchApiSearchTermMoreApps")
								.attr(
										"src",
	...
	private native void searchReviews(String trackId, String countryCode) /*-{
		$wnd.$('#ref-customerReviewsRssRequest').remove();
		$wnd.$('body').append(
				$wnd.$("<script>").attr("id", "ref-customerReviewsRssRequest")
						.attr(
								"src",
								"http://itunes.apple.com/" + countryCode
										+ "/rss/customerreviews/id=" + trackId
	...
	private native void searchAppForRatingByCountry(String trackId, String countryCode) /*-{
		...
		$wnd.$('body').append(
				$wnd.$("<script>").attr("id",
						"ref-searchApiLookupGetAppDetails").attr(
						"src",
						"https://itunes.apple.com/" + countryCode
								+ "/lookup?id=" + trackId
								+ "&callback=handleAverageRatings"));
	}-*/;
		...
		if (jelement != null) {
			...
			if (jobject != null) {
				...
				if (jarray != null && jarray.size() > 0) {
					...
					AppDetails.INSTANCE.setAppDetails(jobject);
					if (jobject.get("artistId") != null) {
						AppDetails.INSTANCE.artistId = jobject.get("artistId").toString();
					}
					if (jobject.get("artistName") != null) {
						AppDetails.INSTANCE.searchMoreApps(jobject.get("artistName").toString());
					}

					if (jobject.get("trackId") != null) {
						AppDetails.INSTANCE.searchReviews(jobject.get("trackId").toString(), String.valueOf(AppDetails.INSTANCE.countrySelector.getSelectedValue()));
					}
				}
			}
		}
			...
			if (jobject != null) {
				...
				if (jarray != null && jarray.size() > 0) {
					...
					for (int a = 0; a < jarray.size(); a++) {
						final JsonObject thisResult = jarray.get(a).getAsJsonObject();
						if (thisResult.get("artistId").toString() == AppDetails.INSTANCE.artistId) {
							filteredArray.add(thisResult);
						}
					}

					if (filteredArray != null && filteredArray.size() > 0) {
						AppDetails.INSTANCE.moreAppsListContainer.getStyle().setProperty("display", "block");
						AppDetails.INSTANCE.populateMoreApps(filteredArray);
					}
				}
			}
		}
	...
	public static void processReviewsResponse(String response) {
		...
		if (jelement != null) {
			final JsonObject jobject = jelement.getAsJsonObject();
			if (jobject != null) {
				final JsonObject jobjectFeed = jobject.getAsJsonObject("feed");
				final JsonArray entries = jobjectFeed.getAsJsonArray("entry");
				if (entries != null && entries.size() > 0) {
					AppDetails.INSTANCE.reviewsContainer.getStyle().setProperty("display", "block");
					AppDetails.INSTANCE.populateReviews(entries);
				} else {
					AppDetails.INSTANCE.reviewsContainer.getStyle().setProperty("display", "none");
				}
			} else {
				AppDetails.INSTANCE.reviewsContainer.getStyle().setProperty("display", "none");
			}
		...
		if (jelement != null) {
			...
			if (jobject != null) {
				...
				if (jarray != null && jarray.size() > 0) {
					...
					AppDetails.INSTANCE.handleAverageRatings(jobject);
				} else {
					AppDetails.INSTANCE.setRatingsToZero();
				}
			}
		}
	}
