...
('system','SGE','Sales gather: generic error', 'Send to admin to inform of the failed sales gather due to a generic error','high','The data account ${dataaccount.username} - id: ${dataaccount.id} gather is failed on ${dataaccountfetch.date} with this message:\n${dataaccountfetch.data}.','Data account ${dataaccount.username} sales gather error','-')$$