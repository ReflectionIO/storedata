...
(function($) {
	...
	window.generateMap = function() {
		window.refMap = new reflectionMap(), $win = $(window), mapTop = $(
				'.landing-section-map').offset().top, dropped = false;

		if ($('.no-touch').length > 0
				&& $win.scrollTop() < (mapTop - ($win.height() / 2))) {
	...
	if ($('.js-scroll-to-anchor').length > 0) {
		$('.js-scroll-to-anchor')
				.click(
						function(e) {
							e.preventDefault();
							var anchor = $(this).attr("href"), navHeight = 60, targetTop = $(
									anchor).offset().top;
							$('html, body').animate({
	...
	if ($('.window-height-scroll-effect-container').length > 0
		...
		if (windowHeight >= breakpointVertical
				&& windowWidth >= breakpointHorizontal) {
			$(window)
					.scroll(
							function() {
								var scrollTop = $(window).scrollTop(), scrollAsPercentageOfWindowHeight = (scrollTop / windowHeight) * 50, scale = 1 - (scrollAsPercentageOfWindowHeight / 100), opacityScale = 1 - ((scrollAsPercentageOfWindowHeight * 2.5) / 100), opacityScaleShadowLayer = 0 + ((scrollAsPercentageOfWindowHeight * 2) / 85);

								$('.landing-section-main').css(
										{
											...
											'transform' : 'scale(' + scale
													+ ')',
											'top' : -scrollTop / 4.3 + 'px',
											'opacity' : opacityScale
										});
								$('.section-size-shadow-layer').css('opacity',
										opacityScaleShadowLayer);
							});
		}
	}
